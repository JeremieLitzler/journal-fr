...
module.exports = {
  ...
  BuildIndex(pagesData) {
    ...
    pagesData.forEach((page) => {
      this.BuildPageIndex(page, searchIndex)
    });
    if (enableLogs) {
  ...
  BuildPageIndex(page, searchIndex) {
    const cleanedPageContent = this.GetCleanedPageContent(page);
    const cleanedWords = this.GetCleanedWords(cleanedPageContent)
  ...
  GetCleanedPageContent(page){
    ...
    content = this.CleanWindowsNewlineToSpace(content);
    content = this.CleanPunctuationToSpace(content);
    return content;
  },
  ...
  GetCleanedWords(pageContent){
    const words = this.ContentToWordsArray(pageContent);
   ...
   */
  DistributeHashedWords(searchIndex, words, currentPage) {
    // => add in hash table each word if absent
    words.forEach((word) => {
      if (word === "") return;
      let lowerCase = word.toLowerCase();
      const normalizedWord = this.RemoveAccents(lowerCase);
      ...
      if (
        searchIndex[key].pages.findIndex((page) => page.slug === slug) === -1
      ) {
